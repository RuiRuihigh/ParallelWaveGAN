...
"""Extract Sony Phonological-Tokenizer tokens + x-vector speaker embedding
and dump them as ParallelWaveGAN training features.

Unlike preprocess_hubert.py (which reads a pre-computed kaldi-style text file
of discrete units), this script calls the tokenizer directly on each wav, and
extracts a per-utterance x-vector (ECAPA-TDNN, speechbrain) instead of doing a
closed-set speaker-id embedding lookup. The x-vector is repeated along the
time axis and concatenated to the token column, so the dumped "feats" array
has shape (T, 1 + spk_embed_dim) and can be consumed as-is by the generic
aux-feature Collater in parallel_wavegan/bin/train.py.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Preprocess audio and extract Phonological-Tokenizer token + "
            "x-vector features."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="speechbrain source model for x-vector extraction.",
    )
    parser.add_argument(
        "--xvector-savedir",
        type=str,
        default="downloads/spkrec-ecapa-voxceleb",
        help="local cache dir for the speechbrain x-vector model.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="cpu",
        help="device used to run the tokenizer and x-vector extractor.",
    )
    parser.add_argument(
        "--verbose",
        type=int,
    ...
    xvector_model = EncoderClassifier.from_hparams(
        source=args.xvector_model,
        savedir=args.xvector_savedir,
    ...
    for utt_id, (audio, fs) in tqdm(dataset):
        ...
        # tokenizer and x-vector extractor both expect a 16kHz wav file on disk
        ...
        # extract x-vector: (spk_embed_dim,)
        with torch.no_grad():
            signal = xvector_model.load_audio(tmp_wav_path)
            xvector = xvector_model.encode_batch(signal.unsqueeze(0))
            xvector = xvector.squeeze(0).squeeze(0).cpu().numpy()

        os.remove(tmp_wav_path)

        tok_col = tokens.reshape(-1, 1).astype(np.float32)
        xvec_rep = np.repeat(
            xvector.reshape(1, -1).astype(np.float32), len(tok_col), axis=0
        )
        feats = np.concatenate([tok_col, xvec_rep], axis=1)
