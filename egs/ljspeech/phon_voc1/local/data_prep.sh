...
# make text (ground-truth transcript) for dev/eval only -- needed by the
# whisper_wer VERSA metric in stage 4, not used anywhere in stage 0-3.
# metadata.csv format: <id>|<transcript>|<normalized_transcript>
for name in "${dev_set}" "${eval_set}"; do
    awk -F'|' 'NR==FNR {want[$1]; next} $1 in want {print $1, $3}' \
        <(awk '{print $1}' "${data_dir}/${name}/wav.scp") \
        "${db_root}/metadata.csv" \
        | sort > "${data_dir}/${name}/text"
done
