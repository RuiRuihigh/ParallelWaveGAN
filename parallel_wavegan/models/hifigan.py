...
class DiscreteSymbolXvectorHiFiGANGenerator(torch.nn.Module):
    """Discrete Symbol HiFiGAN generator conditioned on a continuous speaker embedding.

    Same as DiscreteSymbolHiFiGANGenerator, but the speaker branch is a linear
    projection of an externally computed continuous embedding (e.g., x-vector)
    instead of a lookup table over a closed set of speaker ids. This allows
    conditioning on speakers that were not seen during training.
    ...
    def __init__(
        ...
        """Initialize DiscreteSymbolXvectorHiFiGANGenerator module.

        Args:
            in_channels (int): Number of input channels (content token embedding dim).
            out_channels (int): Number of output channels.
            channels (int): Number of hidden representation channels.
            num_embs (int): Discrete symbol size.
            spk_embed_dim (int): Dimensionality of the externally computed speaker
                embedding (e.g., x-vector) that is fed in as part of the input.
            spk_emb_dim (int): Dimension after projecting the speaker embedding.
            concat_spk_emb (bool): Whether to concat speaker embedding to the input.
            kernel_size (int): Kernel size of initial and final conv layer.
            upsample_scales (list): List of upsampling scales.
            upsample_kernel_sizes (list): List of kernel sizes for upsampling layers.
        ...
        # define id embedding and speaker projection
        self.emb = torch.nn.Embedding(num_embeddings=num_embs, embedding_dim=in_channels)
        self.spk_proj = torch.nn.Linear(spk_embed_dim, spk_emb_dim)
        if not concat_spk_emb:
            assert in_channels == spk_emb_dim
        else:
            in_channels = in_channels + spk_emb_dim
    ...
    def forward(self, c):
        ...
        Args:
            c (Tensor): Input tensor (B, 1 + spk_embed_dim, T). Channel 0 is the
                discrete content token id, channels 1: carry the speaker
                embedding repeated identically along the time axis.

        Returns:
            Tensor: Output tensor (B, out_channels, T).

        """
        assert c.size(1) == 1 + self.spk_embed_dim
        c_idx = c[:, 0:1, :].long()
        # speaker embedding is repeated along time in preprocessing, so any
        # single frame carries the full vector
        xvec = c[:, 1:, 0]  # (B, spk_embed_dim)
        c = self.emb(c_idx.squeeze(1)).transpose(1, 2)  # (B, C, T)
        g = self.spk_proj(xvec)  # (B, spk_emb_dim)

        # integrate speaker embedding
        if not self.concat_spk_emb:
            c = c + g.unsqueeze(2)
        else:
            g = g.unsqueeze(2).expand(-1, -1, c.size(2))  # (B, spk_emb_dim, T)
            c = torch.cat([c, g], dim=1)  # (B, C + spk_emb_dim, T)
    ...
    def inference(self, c, normalize_before=False):
        ...
        Args:
            c (Union[Tensor, ndarray]): Input tensor (T, 1 + spk_embed_dim).
